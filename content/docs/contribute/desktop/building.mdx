...
We've taken the time to make building Enso Browser as easy as possible, independent of your operating system or technical knowledge.
...
First, let's get your system ready for building Enso.

<Tabs groupId="toolbar" items={['Windows', 'Linux', 'macOS']} persist>
  <Tab value="Windows">
      <p>Follow the steps below to set up your Windows environment for building Enso Browser:</p>
      <ol>
  ...
  <Tab value="Linux">
      <p>Follow the steps below to set up your Linux environment for building Enso Browser:</p>
      <ol>
  ...
  <Tab value="macOS">
      <p>Follow the steps below to set up your macOS environment for building Enso Browser:</p>
      <ol>
...
After having everything set up, you need to clone the Enso Browser repository to your local machine. This will create a local copy of the project that you can work on.

```bash
git clone https://github.com/enso-browser/desktop.git --depth 10
cd desktop
```
...
This command will install all the packages listed in the `package.json` file, which are required for building and running Enso Browser.
...
This script updates the "en-US" localization files, which are necessary for proper language support in Enso Browser. Running this step ensures that your build includes the latest translations and language resources.
...
This command compiles the source code and creates the necessary files for running Enso Browser.
...
#### Q: "enso.exe" not found after build?  
